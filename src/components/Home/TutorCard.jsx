...
const TutorCard = ({ tutor }) => {
    ...
    return (
        ...
        <div className="card-quiet group h-full flex flex-col relative overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)]">
            ...
            <div className="aspect-[3/2] overflow-hidden bg-[var(--color-surface-muted)] border-b border-[var(--color-border)] relative">
                ...
                <div className="absolute top-3 right-3 z-20">
                    <span className="px-2 py-1 bg-[var(--color-surface)]/90 backdrop-blur-sm text-[10px] font-bold uppercase tracking-wider text-indigo-700 rounded-md shadow-sm">
                        Pro Tutor
                    </span>
                </div>
            ...
            <div className="p-4 flex-grow flex flex-col">
                <div className="mb-3">
                    <div className="flex items-center justify-between gap-4 mb-2">
                        <h3 className="text-lg font-extrabold text-[var(--color-text-primary)] truncate group-hover:text-teal-600 transition-colors">{displayName}</h3>
                        <div className="flex items-center gap-1 shrink-0 bg-yellow-500/10 px-2 py-0.5 rounded-full border border-yellow-500/20">
                    ...
                    {subjects && Array.isArray(subjects) && subjects.length > 0 && (
                        <div className="flex flex-wrap gap-2 mt-4">
                            {subjects.slice(0, 3).map((sub, i) => (
                                <span key={i} className="px-2.5 py-1 bg-[var(--color-surface-muted)] border border-[var(--color-border)] text-[10px] font-bold text-[var(--color-text-secondary)] uppercase tracking-tight rounded-md group-hover:bg-indigo-500/10 group-hover:text-indigo-400 group-hover:border-indigo-500/20 transition-colors">
                                    {sub}
                                </span>
                            ))}
                        </div>
